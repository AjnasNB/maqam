...
The final 0.3.1 guarded-tool and governed-browser implementation, supported-runtime policy, and release workflow landed first as clean `main` commit `a96413c4da5f27dc31b9772996e70faab0b38382`. The raw performance, conformance, and digest-manifest outputs were then copied unchanged into a separate evidence-only pull request. That follow-up changes no fingerprinted implementation or benchmark source, so the measured commit remains an ancestor of the candidate release commit.

[`2026-07-19-mges-performance-ubuntu24-node24-main-a96413c4.json`](results/2026-07-19-mges-performance-ubuntu24-node24-main-a96413c4.json) records the passing annotation-free exact-main candidate generated by [workflow run 29696564034](https://github.com/AjnasNB/maqam/actions/runs/29696564034):

| Field | Observed value |
|---|---:|
| Environment | Node 24.18.0, GitHub-hosted Ubuntu 24.04 x64, AMD EPYC 7763 |
...
The matching [conformance artifact](results/2026-07-19-mges-conformance-ubuntu24-node24-main-a96413c4.json) records **14/14** passing project-defined fixtures from the same clean commit. The performance artifact SHA-256 is `977f9fcd2ef447840cd78d0e4c04ae937d8049644d3a027d52d6ff59a61b3d57`; the conformance artifact SHA-256 is `44b78de19625ba6337bad7508ffafba938a613c57895f3c55c5fd2f6eeb5675a`. The unchanged [digest manifest](results/2026-07-19-mges-evidence-manifest-ubuntu24-node24-main-a96413c4.json), SHA-256 `4b6d26a0f303c312124685cfa8ea0e257caf09e7f07db7d7bb9298301d4dd974`, binds both files to the commit, clean checkout, runtime, and runner class.

An earlier exact-main run at commit `29c1b9ec0fb8af162d1b73f950851263d35a0527` also passed but emitted a Node 20 deprecation annotation from its artifact-upload action runtime. It is truthfully retained as a superseded passing run—never as a failed or REVIEW result—through its raw [performance](results/2026-07-19-mges-performance-ubuntu24-node24-main-29c1b9ec.json), [conformance](results/2026-07-19-mges-conformance-ubuntu24-node24-main-29c1b9ec.json), and [manifest](results/2026-07-19-mges-evidence-manifest-ubuntu24-node24-main-29c1b9ec.json). The action runtime was upgraded, and the annotation-free `a96413c4` run above is the final candidate evidence.

The earlier metadata-only clean-main candidate at commit `513a7a0bf3711e26ca0e82b4ae1a1663553cc345` remains transparent historical evidence: its [passing performance](results/2026-07-19-mges-performance-windows-node24-main-513a7a0b.json) and [conformance](results/2026-07-19-mges-conformance-windows-node24-main-513a7a0b.json) records are not relabeled for the guarded-browser source. Three preceding performance attempts from that same commit are retained rather than hidden: [attempt 1](results/2026-07-19-mges-performance-windows-node24-main-513a7a0b-review-attempt1.json) observed `14.223%` governed CV, [attempt 2](results/2026-07-19-mges-performance-windows-node24-main-513a7a0b-review-attempt2.json) observed `14.860%`, and [attempt 3](results/2026-07-19-mges-performance-windows-node24-main-513a7a0b-review-attempt3.json) observed `12.886%`. Each is labeled `REVIEW` because it exceeded the unchanged 10% stability threshold. No observations or thresholds were removed.
...
The previous public 0.3.0 clean-main release artifact [`2026-07-18-mges-conformance-windows-node24-main-545fe8bb.json`](results/2026-07-18-mges-conformance-windows-node24-main-545fe8bb.json) records **14/14 passing project-defined fixtures**:
...
### Compact, acceptable form for the 0.3.1 pre-publication candidate evidence

> 0.3.1 pre-publication candidate evidence — MGES v1.1.0 local-call profile on Node 24.18.0 / Ubuntu 24.04 x64 / AMD EPYC 7763: 129.849 microseconds median per governed call (95% bootstrap interval for the sample median: 129.539-130.648; 30 fresh-process observations; governed CV 1.111%; required checks PASS). Measured from exact clean main commit `a96413c4da5f27dc31b9772996e70faab0b38382`. Local in-process component benchmark; excludes model, network, storage, browser and concurrency; not a competitor benchmark, release announcement or SLA. Raw JSON: [artifact](results/2026-07-19-mges-performance-ubuntu24-node24-main-a96413c4.json).

For conformance:

> 0.3.1 pre-publication candidate evidence — MGES v1.1.0 governance-boundary profile: 14/14 project-defined fixtures passed on the clean-main source fingerprint. This is regression evidence, not a browser-adapter test, security score, penetration test or certification. Raw JSON: [artifact](results/2026-07-19-mges-conformance-ubuntu24-node24-main-a96413c4.json).

The previous public 0.3.0 wording and raw files remain in the [0.3.0 release record](../docs/release-0.3.0.md) and the previous-public section above. Do not silently substitute candidate values into an already published historical record.
