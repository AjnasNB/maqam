...
Publishing requires explicit user approval for the exact package, version, registry, command, artifact identity, and Git commit in the current run. Approval must name `maqam@0.2.1` or a later reviewed version and use the `publish:npm` action.
...
```js
{
  ...
  artifactFilename: "maqam-0.2.1.tgz",
