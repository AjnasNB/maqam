...
The release artifact records clean post-squash source commit `545fe8bbc40f21cec0f9ec2ae3954f3e75783f22` with `workingTreeDirty: false` and fingerprints every benchmark and implementation file in the measured path. The evidence-only release change does not alter those fingerprinted files; any later fingerprinted change requires another run.
...
- [Raw clean-main performance JSON](../benchmarks/results/2026-07-18-mges-performance-windows-node24-main-545fe8bb.json)
- [Raw clean-main conformance JSON](../benchmarks/results/2026-07-18-mges-conformance-windows-node24-main-545fe8bb.json)
...
> MGES v1.1.0 local-call release profile on Node 24.15.0 / Windows x64 / Ryzen 7 4800H: 140.816 microseconds median per governed call (95% bootstrap interval for the sample median: 138.983-142.820; 30 fresh-process observations; governed CV 5.020%; required project checks PASS). Measured from clean post-squash main commit `545fe8bbc40f21cec0f9ec2ae3954f3e75783f22`. Local in-process component benchmark; excludes model, network, storage and concurrency; not a competitor benchmark or SLA.
