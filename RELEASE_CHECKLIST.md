...
Use an authenticated npm session or a short-lived release token without writing credentials into the repository, shell history, package metadata, logs, or approval evidence. Record the published npm version and registry integrity, then create a matching GitHub release only after the npm publish succeeds.
