...
Use this root checklist as the release gate for `maqam@0.3.0`. The historical 0.2.4 artifacts remain evidence for 0.2.4 only.
...
- MGES conformance and performance are rerun when fingerprinted source or lockfile inputs change; 0.2.4 figures are not relabeled as 0.3.0 evidence.
...
- `npm pack --dry-run` includes `src/`, `src/index.d.ts`, `src/maqam/server.d.ts`, `examples/`, `docs/governed-sources.md`, `docs/migration-0.3.md`, `docs/release-0.3.0.md`, the other public docs, `README.md`, `LICENSE`, `CHANGELOG.md`, `SECURITY.md`, `RELEASE_CHECKLIST.md`, and `LICENSE_AUDIT.md`.
...
Do not start the trusted publishing job until the exact artifact receives explicit maintainer approval for `maqam@0.3.0` and the protected GitHub environment can enforce that approval.
