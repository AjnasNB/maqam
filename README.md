...
|---|---|
| `maqam@0.3.0` | One compact boundary around tool calls, exact approvals, worker adapters, traces, evidence, governed source routing, anonymous web search, public YouTube metadata/captions, or bounded HTTP/feed research |
...
The project-defined [Maqam Governance Evaluation Suite (MGES) v1.1.0](benchmarks/README.md) keeps performance and conformance separate. The clean post-squash `main` 30-observation Windows/Node 24 local-call result is `140.816 microseconds/call` median, with a `138.983-142.820 microseconds/call` 95% bootstrap interval for the sample median and `5.020%` governed coefficient of variation. The separate governance profile passes `14/14` named fixtures, including two governed-source routing cases.

Those figures are local release regression evidence, not a globally standardized benchmark, cross-product speed comparison, security score, certification, network benchmark, or production SLA. Read the [raw artifacts, complete methodology and publication wording](benchmarks/README.md) or the detailed article, [Benchmarking an agent-governance boundary without fooling yourself](docs/articles/benchmarking-agent-governance.md), before quoting them. The artifacts record clean source commit `545fe8bbc40f21cec0f9ec2ae3954f3e75783f22`; this evidence-only change does not alter fingerprinted source, and any later fingerprinted change requires another run.
...
Maqam 0.3.0 is public on npm with matching trusted-publishing provenance, registry `gitHead`, integrity, Git tag, and GitHub release records. Source metadata alone does not identify the current npm dist-tag, so verify `npm view maqam dist-tags.latest gitHead dist.integrity` and the repository's [latest release](https://github.com/AjnasNB/maqam/releases/latest) before installation. Do not republish an existing version. New releases use the maintainer-approved [trusted npm publishing workflow](https://github.com/AjnasNB/maqam/actions/workflows/publish-npm.yml), which rebuilds and verifies the exact approved artifact before npm's OIDC publisher accepts it.
